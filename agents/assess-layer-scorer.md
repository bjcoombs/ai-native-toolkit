...
- **`instruction_claims.failed` > 0 → cite each failure as Layer 0 evidence; cap the instruction half at Partial.** The core extracted the checkable claims in the graded instruction files and verified each against the repo: `kind: enforcement` is a backticked script the file says is enforced in CI that no file under `.github/workflows/` references; `kind: pin` is a "pinned in `<file>`" version the file does not hold (or a pinned file that does not exist). Each failure names `file`, `line`, `path` (the script or pinned file) and, for a pin, `version`. A well-graded file that states false facts about its own enforcement is a lying map, so name it ("`AGENTS.md:5` says `scripts/check-x.sh` is enforced in CI; no workflow calls it") and make the action "wire it into CI or delete the sentence". `total: 0` means no checkable claim was found - no evidence either way, not a clean bill.
