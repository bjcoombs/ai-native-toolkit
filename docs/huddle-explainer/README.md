...
A roughly two-minute narrated explainer for `/huddle`, built to show someone unfamiliar with the skill why structured multi-perspective deliberation is a different thing from asking one model once. The visual asset lives in a Claude Design project (linked below); this folder carries the narration audio and the production story, because the story turned out to be a fitting demonstration of the thing the video is arguing for. Back to the [Map of Content](../index.md).

## The asset

- **Visual**: [Huddle Visualization](https://claude.ai/design/p/b886609f-11c3-4c07-a50a-0ec439b6f479?file=Huddle+Visualization.dc.html) - an animated timeline built in Claude Design (`.dc.html` plus two `.jsx` files), showing an open-ended problem move through framing, Fibonacci sizing, the five hats in sequence, forced cross-talk, a bounded completeness-critic pass, and a verdict that keeps its dissent attached rather than smoothing it away.
