...
Every command below is recorded verbatim with its exit code. Output is untidied, with two disclosed exceptions: timestamps are stripped from the `[DEBUG]` lines in Probe 2 (noted again there), and absolute home-directory paths and file owners are redacted - the worktree path to `<repo>`, the plugin cache path to `~`, and the `ls -laR` owner and group columns to `<user>` and `<group>`. Nothing else was altered. Three departures from that follow, each labelled where it appears: the `ls -laR` listing in Probe 1 and the **Derived summary** table in Probe 2 are derived rather than verbatim, and the bare `claude --print "/probeskill"` run near the end of Probe 2 was executed but never captured, so no command line or exit code exists for it.
...
Both symlinks dereference, and the `claude plugin details ant-umbrella@ant-probe` transcript above settles it on its own: a full capture with exit code 0, reporting `Skills (1)  probeskill` and `Agents (1)  probe-agent`, each symlinked component listed exactly once. The `agents/` dereference the plugins-reference does not document therefore works the same way `skills/` does. The cache listing corroborates at a lower grade: `agents/probe-agent.md` and `skills/probeskill/SKILL.md` appear as regular files (mode `-rw-r--r--`, not `lrwxr-xr-x`), but that comes from the derived listing above, whose invocation and exit code were not recorded.

What the probe measured is loading and inventory: both symlinked components load under the umbrella and are listed once each. What it did not exercise is name resolution. The only registry ids recorded anywhere in this file - Probe 2's derived summary - are namespaced, `ant-assess:probe-agent` and `ant-umbrella:probe-agent`; no bare `probe-agent` entry was reported, and no bare `subagent_type` lookup was invoked. D5's "bare agent names" claim therefore rests on the documented Claude Code plugin-agent lookup, not on this probe, and WS1 verifies it from a GitHub-sourced marketplace.

No `README` component appears in the inventory; note the weaker half of that finding, which the probe does not strengthen: `README.md` sat at the plugin root, where it was never a component. The repo's current `README` leak comes from `commands/README.md` and `agents/README.md`, and this probe did not reproduce that shape. D5 is marked Decided on the dereference finding; R3 still owns removing the two component-directory READMEs.
