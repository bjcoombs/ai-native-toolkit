...
- **Marked component.** Any path under the component directory of a marked file at `$BASE_SHA` or at `HEAD`, where "marked" is the standalone-anchor test the `markers` subcommand applies - a line whose stripped content is the bare marker, the definition `standalone_anchor_count` holds at `scripts/floor_check.py:78-88` - and not a raw token match. For a marked `skills/<x>/SKILL.md` or `plugins/<p>/skills/<x>/SKILL.md` the component is the parent directory and everything beneath it; for a marked `commands/<x>.md` the component is that file. This keeps `skills/marathon/forge/` protected, as today's `skills/marathon/` prefix does. The two definitions have to agree or this spec is its own counterexample: it quotes the marker several times in fenced blocks and inline code and carries no standalone anchor line, so a `grep -c -F` for the marker over this file returns a non-zero count while `grep -c -x '[[:space:]]*<!-- floor:cold-verify-completion -->[[:space:]]*'` returns 0. A raw-token `protected` would call it marked, and its component directory would be undefined because a `docs/` path matches none of the three component shapes above, so every later edit to this spec would demand a floor sign-off. Under the standalone-anchor test it is a prose carrier and nothing more.
...
Clause iii's "out-of-band sign-off" gets one artefact: a `floor-signoff` GitHub Environment with the repository owner as its sole required reviewer. The measurement above confirms no such environment exists yet, so PR0 creates it. `floor.yml` gains a `floor sign-off` job, job id `signoff`, carrying `environment: floor-signoff` and running only when `protected` reports a floor core file or a token change. `floor enforcement` gains `needs: [signoff]`, the guard `if: ${{ !cancelled() }}`, and a first step, `Fail on a refused sign-off`, that reads `needs.signoff.result` and exits non-zero with a named reason when it is `failure`.

The guard is not optional, and it is `!cancelled()` alone. GitHub propagates a skipped `needs` job to its dependents, and branch protection reads a skipped required context as satisfied - so an unguarded `needs` would let any PR that touches nothing protected skip both jobs and go green with removal detection and clause integrity never having run. That is the invariant `.github/workflows/floor.yml:115-116` already records: floor.yml must keep running on all PRs for the two deterministic jobs.

Adding `needs.signoff.result != 'failure'` to that guard reopens the same hole from the other end. GitHub records a refused deployment review as a `failure` result on `floor sign-off`, so the extra conjunct would make `floor enforcement` skip in exactly the case the sign-off exists to stop, and a skipped required context reads as satisfied. `floor self-anchor` declares no `needs` on `signoff`, so it stays green as well, and a refused sign-off would leave both floor contexts satisfied and the PR mergeable. Refusal has to make the required context red rather than absent: `floor enforcement` runs on every non-cancelled outcome and its first step converts `needs.signoff.result == 'failure'` into a job failure naming the refusal. The job id carries no hyphen so `needs.signoff.result` parses as property access rather than subtraction.
...
2. `action.yml` gains `outputs.rendered` sourced from the `render` step; `assess-gate.yml` gives its `uses: ./` step an `id` and asserts `steps.<id>.outputs.rendered != 'failure'` for this repo's own self-test. Reading `steps.render.outcome` from the caller does not work: `render` is a step inside the composite (`action.yml:81`), so the name is not in the caller's scope. The caller can address only its own `uses: ./` step, and that step's outcome is decided by the composite's final `Gate on floors and regressions` step (`action.yml:107-118`, documented there as the single step whose exit code can fail the consumer's PR), so it reports the gate verdict and says nothing about whether the snapshot rendered. Asserting equality with `success` does not work either: `render` is skipped rather than failed when uv is unavailable (`action.yml:87`), so a transient install failure would turn the gate red against the warn-only contract documented at `action.yml:82-86`. Consumers stay warn-only, because the assertion lives in the workflow, not the composite.
...
8. The role classification loses nothing the regex protects. The check is fixture-based rather than pinned to a live SHA, because it runs in `scripts/ pytest`, whose checkout sets no `fetch-depth` (Current state) and so resolves nothing but the tip commit; `git ls-tree -r 44bddbf` would error there. Two path lists ship under `scripts/tests/fixtures/`, generated once from the tree the measurements above were taken on and committed beside the test:

   ```
   git ls-tree -r --name-only 44bddbf > scripts/tests/fixtures/floor_roles_tree.txt
   git ls-tree -r --name-only 44bddbf \
     | grep -E '^(skills/marathon/|skills/pr-review-merge/|commands/tm\.md$|commands/issues\.md$|scripts/contract/|scripts/canaries/|tests/canaries/|\.github/workflows/floor\.yml$)' \
     | sort > scripts/tests/fixtures/floor_roles_regex.txt
   ```

   The test materializes `floor_roles_tree.txt` as empty files in a scratch repository - the same construction the R100, R099 and `D` plus `A` cases use - writes a standalone anchor line into the four marked files and a fenced prose mention of the marker into a fifth, commits, then pipes the same path list through `protected --base <scratch HEAD> --changed`. The returned set is `floor_roles_regex.txt` plus exactly three files - `FLOOR.md`, `scripts/floor_anchor.py`, `scripts/floor_check.py` - with nothing dropped. In particular `scripts/contract/verifier.py`, `scripts/contract/tiers.py` and `scripts/canaries/drive_interactive.mjs` are classified `protected`, and the prose-only carrier is not: it holds no standalone anchor, so it is no marked component and neither it nor its directory is returned.
...
12. `floor enforcement` runs on every non-cancelled outcome of `floor sign-off` and is never skipped by it. The job carries `if: ${{ !cancelled() }}` with no result conjunct, and its first step exits non-zero when `needs.signoff.result` is `failure`. Two observations, each read from `gh api repos/bjcoombs/ai-native-toolkit/actions/runs/<run-id>/jobs | jq '[.jobs[] | {name, conclusion}]'`:

    - Skipped sign-off, on a PR touching nothing protected: `floor sign-off` has conclusion `skipped` and `floor enforcement` has conclusion `success` - the job ran, it was not skipped - and the `floor enforcement` log carries the `merge-base:` line its removal-detection step prints.
    - Refused sign-off, on a PR touching the floor core whose deployment review is rejected: `floor sign-off` has conclusion `failure` and `floor enforcement` has conclusion `failure`, not `skipped`, with its first step's log naming the refusal. The required context is red, so branch protection holds the merge instead of reading an absent context as satisfied.
...
- **`scripts/ pytest`** (required context) covers requirements 1-6, 8 and 15. New cases in `scripts/tests/test_floor_check.py`: discovery returns the four-file set from a fixture repository whose sixth candidate carries only a prose mention; the R100 pair passes; the R099 anchor-strip pair fails; the truncated `D`/`A` pair fails; the `docs/archive/` destination fails the structural check; a `FLOOR.md` whose token block is deleted fails `clauses`; and requirement 8's set comparison runs against its committed path-list fixtures in a scratch repository, asserting the three added files, no dropped file, and no marked-component role for the prose-only carrier, so a later narrowing of the roles goes red here. No case in this job resolves a historical SHA, which is what keeps them runnable in a checkout with no `fetch-depth`.
...
- **`floor enforcement`** (required context) runs the rewritten `markers` and `clauses` subcommands plus the new `protected` call against PR0 itself. PR0 touches the floor core, so its own run cannot demonstrate requirement 12; the skip observation comes from a nothing-protected PR and the refusal observation from a floor-core PR whose review is rejected.
...
- **Recorded verdicts** cover requirements 10, 11 and 12: `gh api repos/bjcoombs/ai-native-toolkit/environments` and the deployment review record for PR0 are pasted into the PR body. Requirement 11's "no new required check" is confirmed by re-running the `required_status_checks` query in Current state and showing the same six contexts. Requirement 12 is confirmed twice, on throwaway branches opened against PR0's head: a docs-only branch, whose Floor run lists `floor sign-off` skipped and `floor enforcement` succeeded; and a branch editing `FLOOR.md` whose deployment review is rejected, whose Floor run lists `floor sign-off` failed and `floor enforcement` failed rather than skipped. Both listings are pasted into PR0's body beside the others, and both branches are closed unmerged.
