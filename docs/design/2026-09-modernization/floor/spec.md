...
76:    return sorted(COMMANDS.glob("*.md")) if COMMANDS.is_dir() else []
...
A new subcommand, `floor_check.py protected --base $BASE_SHA --changed`, reads changed paths on stdin or from `git diff --name-only` and decides protection by role rather than by literal path: a marked file at `$BASE_SHA` or at `HEAD`, a gate script matched by basename (`start_gate.py`, `spawn_verifier.py`, `complete_gate.py`, `run_canaries.py`), a canary fixture directory, or a floor core file (`FLOOR.md`, `floor_check.py`, `floor_anchor.py`, `floor.yml`). The eight-alternative regex at `.github/workflows/floor.yml:142` is replaced by a call to it, and `FLOOR.md` is rewritten so no section names a path: the Markers section describes the token and points at the discovery command, and clause iii's list becomes role-based.

### Sign-off artefact

Clause iii's "out-of-band sign-off" gets one artefact: a `floor-signoff` GitHub Environment with the repository owner as its sole required reviewer. The measurement above confirms no such environment exists yet, so PR0 creates it. `floor.yml` gains a `floor sign-off` job carrying `environment: floor-signoff`, running only when `protected` reports a floor core file or a token change; `floor enforcement` gains `needs: [sign-off]`. Merge therefore stays blocked by the two floor contexts that branch protection already requires, so PR0 needs no branch-protection PATCH and adds no required context. The approval is a separate click in the deployments UI with an actor and a timestamp GitHub records outside the diff.
...
`FLOOR.md` clause iii gains the sentence "recorded as the maintainer's deployment review of the floor-signoff environment", `REQUIRED_CLAUSES['iii']` gains the key phrase `floor-signoff`, and clause iii's protected set gains `floor_check.py` and `floor_anchor.py`, which it omits today.
...
2. `action.yml` gains `outputs.rendered` from the `render` step; `assess-gate.yml` gains a step asserting `steps.render.outcome == 'success'` for this repo's own self-test. Consumers stay warn-only, because the assertion lives in the workflow, not the composite.
...
6. `tests/test_plugin_contract.py` discovers components with `rglob` and re-parametrizes `test_subagent_types_resolve` over every `SKILL.md`, so the test does not silently yield zero cases when `commands/` is deleted.
...
7. `floor_check.py protected --base <ref> --changed` classifies a changed-path list by role and is the only path decision `.github/workflows/floor.yml` makes; no path literal from the current regex survives in the workflow.
8. `FLOOR.md` names no file path in its Markers section or in clause iii's protected list; `rg -n 'skills/|commands/|plugins/' FLOOR.md` returns only the fenced token block's own contents.
9. A `floor-signoff` environment exists on the repository with the owner as sole required reviewer, and `gh api repos/bjcoombs/ai-native-toolkit/environments` lists it.
10. `.github/workflows/floor.yml` has a `floor sign-off` job with `environment: floor-signoff`, and `floor enforcement` declares it in `needs`. No job `name:` literal changes and no new required status check is registered.
11. `scripts/floor_anchor.py` fails closed when the environment is absent, when the owner is not a required reviewer, or when the checked-out `floor.yml` no longer wires the environment into a `needs` of `floor enforcement`.
12. `FLOOR.md` clause iii carries the phrase `floor-signoff` and lists `scripts/floor_check.py` and `scripts/floor_anchor.py` among the protected floor core; `REQUIRED_CLAUSES['iii']` asserts the phrase.
13. `scripts/tests/test_floor_check.py:125-126` is re-pointed at a file resolved through discovery rather than the literal `skills/marathon/SKILL.md`.
14. `test_action_contract.py` asserts each `working-directory` value in `action.yml` exists on disk; both current values (`action.yml:89` and `:113`) pass.
15. `action.yml` declares `outputs.rendered` and `.github/workflows/assess-gate.yml` fails when `steps.render.outcome != 'success'`.
16. `scripts/floor_anchor.py` asserts every required context is a job `name:` literal in `.github/workflows/*.yml`; the six contexts measured above pass the subset check.
17. The `plugin contract pytest` job in `.github/workflows/tests.yml` sets `fetch-depth: 0` and fetches the PR base ref.
18. `scripts/floor_anchor.py:106` and `docs/floor-anchor-proof.md:178` both carry a `--app dependabot` rotation line beside the default one.
19. `tests/test_plugin_contract.py` discovers components with `rglob`, and `test_subagent_types_resolve` is parametrized over every `SKILL.md` in the tree; collecting it yields at least twelve cases on `main`.

## Verification

- **`scripts/ pytest`** (required context) covers requirements 1-6 and 13. New cases in `scripts/tests/test_floor_check.py`: discovery returns the four-file set from a fixture repository whose sixth candidate carries only a prose mention; the R100 pair passes; the R099 anchor-strip pair fails; the truncated `D`/`A` pair fails; the `docs/archive/` destination fails the structural check; a `FLOOR.md` whose token block is deleted fails `clauses`.
- **`plugin contract pytest`** (required context) covers requirements 17 and 19 through `tests/test_plugin_contract.py`, including a collection-count assertion so `test_subagent_types_resolve` cannot regress to zero cases.
- **`skills/assess pytest`** (required context) covers requirement 14 through the fifteenth test in `skills/assess/tests/test_action_contract.py`.
- **`floor enforcement`** (required context) runs the rewritten `markers` and `clauses` subcommands plus the new `protected` call against PR0 itself.
- **`floor self-anchor`** (required context) covers requirements 11 and 16. Its first run on PR0 is expected to fail closed on the environment assertion until the environment is created, which is the fail-closed behaviour the script already has for branch protection; the red is the signal that the out-of-band step is outstanding.
...
- **`AI-readiness regression gate`** covers requirement 15 end to end: after the change, a deliberately broken `working-directory` in a scratch branch turns the job red instead of emitting a notice.
- **Recorded verdicts** cover requirements 9 and 10: `gh api repos/bjcoombs/ai-native-toolkit/environments` and the deployment review record for PR0 are pasted into the PR body. Requirement 10's "no new required check" is confirmed by re-running the `required_status_checks` query in Current state and showing the same six contexts.
