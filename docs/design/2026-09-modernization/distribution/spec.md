...
One line inside that procedure is WS7's regardless of which file holds it when PR7.3 lands. `CLAUDE.md:137` builds `ZIP_URL` as `releases/tag/standalone-skills-v${VERSION}` and `gh release edit` prepends it to every `v*` release's notes. After the rename below, no `standalone-skills-v*` release is ever created again, so that single link is a 404 on the first release cut after WS7, and `README.md:7` sends every standalone user down it. The replacement is one link per plugin that owns a ZIP, up to four `releases/tag/standalone-<plugin>-v<version>` URLs under the same heading, each version read from `plugins/<plugin>/.claude-plugin/plugin.json` at the tagged commit rather than from the `v*` tag. Reading the manifest is what makes each link resolve: a plugin whose version has not moved since its last standalone release still has that release, and its manifest still names that version. Requirement 19.
...
`--latest=false` is kept on every release so `releases/latest` stays the plugin release, which is what the Current state above records and what `README.md:7` depends on: the plugin release's notes are the hand-off to the ZIPs, and a standalone release taking the Latest slot would strand that hand-off. Adopter `uses:` pins and their Dependabot bumps read git tags, not the release listing (`CLAUDE.md:148`), so they resolve the same either way and are not the reason for the flag. The existing-release skip (lines 64 to 67) is kept per plugin, unchanged in intent: an immutable release is never overwritten.
...
The rename and the split reach three `README.md` passages that no other workstream owns. `README.md:188` names the five assets literally (`assess.zip`, `huddle.zip`, `deslop.zip`, `skill-forge.zip`, `semantic-compress.zip`), every one of them renamed by requirement 11. `README.md:188` and `:201` describe one bundle published per plugin version bump, where after WS7 there are up to four per-plugin releases. `README.md:207` tells the reader to compare the Skills UI description suffix `Standalone build vX.Y.Z` against the latest release, a check that cannot succeed once the deslop ZIP carries the deslop plugin's version and the `v*` tag tracks assess alone. These are prose claims rather than repo paths, so R11's consumer table does not reach them, and Breadcrumbs below covers only the historical rows in `docs/migration-2.0.md`. PR7.3 rewrites all three in the same PR that changes the naming. Requirement 20.
...
19. The release procedure prepends one `releases/tag/standalone-<plugin>-v<version>` link per plugin that owns a ZIP, each version read from that plugin's manifest, and no `standalone-skills-v` string survives in it. Verified by `rg -c 'standalone-skills-v'` over the file holding the procedure when PR7.3 lands (`CLAUDE.md`, or `plugins/delivery/skills/marathon/references/release-after-marathon.md` once R7 has moved it) reporting no match, and by the prepended links on the first `v*` release cut after WS7 resolving to live release pages.
20. `README.md` names no `<skill>.zip` asset, describes per-plugin standalone releases rather than one bundle, and states a version check a standalone user can actually perform. Verified by `rg -c 'assess\.zip|huddle\.zip|deslop\.zip|skill-forge\.zip|semantic-compress\.zip' README.md` reporting no match, and by the rewritten passages naming `<skill>-v<version>.zip` and the `standalone-<plugin>-v<version>` release pages.
...
3. **PR7.3 - per-plugin versions in the pipeline.** Replaces `VERSION`/`VERSION_SUFFIX` with a per-plugin `version_suffix()`, adds the `plugin` key to each `SKILLS` entry, and repoints the workflow trigger, version compare, and release naming to per-plugin. Rewrites the release procedure's `ZIP_URL` line and the three `README.md` standalone passages in the same PR (requirements 19 and 20), so no doc points at a `standalone-skills-v*` release the workflow no longer creates. May not touch `source_dir` or `bundle_files` (PR1 owns those rows) and may not touch the frontmatter transform.
