...
- When the base ref is unavailable (a local run with no remote, a detached checkout), the test skips with a stated reason rather than passing silently. A skip in CI is a failure of the fetch step, which is why the fetch is a separate named step and not an inline `||`.
...
| 44-46 | `build-standalone-skills.sh` with no argument | one invocation per changed plugin, building only that plugin's ZIPs |
| 48-73 | one release `standalone-skills-v${VERSION}` | one release per changed plugin, `standalone-<plugin>-v<version>`, assets named `<skill>-v<version>.zip` |
...
- PyYAML is installed through `uv run --with pyyaml`, matching how the rest of the pipeline resolves dependencies.
- The pin is a full SHA, never a tag or branch, and the PR that introduces or moves it records what changed between the old and new SHA in its body. This is the first third-party code this repo runs in a job holding `contents: write`, so the pin is reviewed the way a dependency is.
...
8. Every plugin has a `CHANGELOG.md`, and a version move without a new section for that version fails the contract test (R15). Verified by fixture cases in the same test module.
...
11. Each release is named `standalone-<plugin>-v<version>`, carries only that plugin's ZIPs as `<skill>-v<version>.zip`, and is created with `--latest=false`. Verified by `rg -n 'standalone-|--latest' .github/workflows/build-standalone-skills.yml` and by the first real release the workflow cuts.
...
17. The `quick_validate.py` checkout pins a full 40-character commit SHA with `persist-credentials: false`. Verified by `rg -n -A 6 'anthropics/skills' .github/workflows/build-standalone-skills.yml`.
...
- `tests/test_plugin_contract.py` covers the bump rules (5, 6, 7) and the changelog rule (8) against constructed base-versus-head fixtures, so the assertions are exercised without needing a real PR. The `plugin contract pytest` job proves the base fetch works on a real PR (4).
...
- `build-standalone-skills.yml` is proved by its first real invocation. The `workflow_dispatch` path runs the build, the validator, and a release create against a version that already has a release, so the idempotent skip (lines 64 to 67 today) exercises without publishing. The publishing path is proved by the first family bump after WS7 merges (11, 16).
