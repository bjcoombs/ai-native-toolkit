...
- ``coverage_report``: no line-coverage report was found, so test depth on the
  hotspots is unmeasured. Names the top hotspots to measure first.
...
# How many top hotspots the coverage action names: one per Top 3 slot.
...
def _coverage_gap(coverage_report: dict, top_hotspots: list[dict]) -> dict | None:
    if coverage_report.get("available") is not False:
        return None
    paths = [h["path"] for h in top_hotspots[:MAX_COVERAGE_PATHS] if h.get("path")]
    return {
...
def build_gap_actions(
    ...
    gaps = [
        _coverage_gap(coverage_report or {}, top_hotspots or []),
