...
- **Dangling references (concepts mentioned but lacking a page).** `doc_graph.dangling_links` counts links pointing at docs that don't exist — the structural half of Karpathy's "important concepts mentioned but lacking their own page". A non-trivial count is a navigability gap (the map names rooms that aren't there); name a few and suggest creating the pages or fixing the links. `ambiguous_wikilinks` (a bare `[[name]]` matching several files) is a milder smell worth noting.
