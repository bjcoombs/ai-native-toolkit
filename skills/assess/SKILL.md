...
- `layer_scores` maps each layer id to its band (Missing 0.0 / Partial 0.5 / Present 1.0). Layer 6 must not exceed **0.5** when `mutation_not_run_cap.applies` is true (see Step 3). Include it so the cap is enforced; a legacy input omitting it skips only the Layer 6 check. The optional `evidence` list (the flat `{layer, kind, path[, needle]}` array the scorer returned) is re-checked against the repository: a layer whose entries are all rejected is refused, naming each entry by kind, path and needle; a layer with at least one verified entry finalises, with each rejected entry printed as a warning. Omit it and finalize skips the check.
