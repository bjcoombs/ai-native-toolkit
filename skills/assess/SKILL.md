...
When the deterministic core detects a Maven or Gradle project (a build file plus at least one `.java`, `.kt` or `.scala` file outside platform-wrapper `android/` directories: a Flutter, React Native, Capacitor or Cordova shell is not a JVM codebase) it emits a `capability_offers` block in `run-context.json` - the first proof of the capability-driven flow on a non-enumerated ecosystem. Read it after Step 2c's core run, before scoring, and act on each capability's `state`:
