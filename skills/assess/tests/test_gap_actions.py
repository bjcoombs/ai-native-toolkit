...
def test_gap_actions_coverage_entry_names_top_hotspots() -> None:
    gaps = build_gap_actions(NO_COVERAGE, _docs(1.0), HOTSPOTS)
    assert len(gaps) == 1
...
def test_gap_actions_empty_when_coverage_present_and_docs_reachable() -> None:
    assert build_gap_actions(COVERAGE, _docs(0.8), HOTSPOTS) == []


def test_gap_actions_reachability_entry_below_floor() -> None:
    gaps = build_gap_actions(COVERAGE, _docs(0.3, unreachable=["docs/z.md", "docs/a.md"]), HOTSPOTS)
...
def test_gap_actions_reachability_at_floor_does_not_fire() -> None:
    assert build_gap_actions(COVERAGE, _docs(REACHABILITY_FLOOR), HOTSPOTS) == []


def test_gap_actions_coverage_comes_before_reachability() -> None:
    gaps = build_gap_actions(NO_COVERAGE, _docs(0.1), HOTSPOTS)
...
def test_gap_actions_no_markdown_is_not_a_reachability_gap() -> None:
    ...
    assert build_gap_actions(COVERAGE, no_docs, HOTSPOTS) == []


def test_gap_actions_unavailable_doc_graph_is_not_a_gap() -> None:
    assert build_gap_actions(COVERAGE, {"available": False, "reachability_pct": 0.0}, HOTSPOTS) == []


def test_gap_actions_missing_blocks_degrade_to_empty() -> None:
    assert build_gap_actions(None, None, None) == []
