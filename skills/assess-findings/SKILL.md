...
**Report format** (write this to disk verbatim, filling in the placeholders; the bolded paragraphs inside the folds are instructions for *how* to fill that section, not literal report text):
...
**Score: X / 8 - <maturity-label>** · Keyhole: <paste `keyhole_summary.summary_text` from run-context.json verbatim>
...
<paste the `findings_markdown` string from run-context.json verbatim here - it renders as a `## Cross-Layer Findings (Keyhole Readiness)` block with the eight findings, their paths, their deterministic actions, and the attention list. Do not paraphrase, summarise, reorder, or drop any finding. The block's own `## Cross-Layer Findings (Keyhole Readiness)` line is the section's ONLY heading - never add a framing heading of your own (no `### Cross-Layer Findings (Keyhole Readiness)` above the block; the heading must appear exactly once in the report). If a sentence or two of framing helps a reader, place it directly under the block's heading line, before the first `###` finding; the findings and attention list themselves are copied, not rewritten.>
