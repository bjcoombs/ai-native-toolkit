...
- **Test Signal** ← `test_signal`: `no_covering_test` → "No covering test", `covered_but_hollow` → "Covered but hollow", `unknown_no_coverage` → "Unknown (no coverage)", `unsupported` → "Unsupported (no report, no sibling test)".
