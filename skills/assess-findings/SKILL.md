...
| 7 | Hollow required review | `review_reality` (<R>% of <N> merged PRs reviewed by someone other than the author; <S>% self-merged) | The branch rules require an approving review, so review reads as enforced, while merges land without one - the gate is bypassed as routine |
...
- **L7 - hollow required review:** add the row only when `review_reality.hollow_required_review` is true (review is required and under 20% of sampled merged pull requests carry a review from someone other than the author). Fill from `merged_count`, `reviewed_share` and `self_merged_share` as percentages; the block holds counts and shares only, so name no pull request or account. `available: false` or a null flag means the sample or the rules could not be read - no row.
