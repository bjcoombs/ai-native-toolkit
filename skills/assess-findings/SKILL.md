...
**Existence and wiring claims cite only verified evidence.** A report sentence saying a file or directory exists or is missing, or that something is or is not wired in (a workflow runs the linter, no CI job calls the script), must rest on an entry in the scorecard's `evidence` list and name the path that entry cites. When no verified entry backs such a claim, leave it out rather than restating it from the scorer's prose. The one exception is the scorecard's per-layer Evidence cell, which is never blank, and it has two empty states. When the scorer offered no entries for a layer (or returned no `evidence` list), keep its one-line note there and end it with `(unverified)`, so the gap reads as a limit of this run rather than as a fact. When a layer's entries were offered and every one was rejected, drop the scorer's note, since repeating it restates a claim the check refuted, and render the gap instead: `no verified evidence - N claim(s) rejected`, followed by each rejected entry's `reason` from `evidence_rejected`. The deterministic findings you paste verbatim are already grounded and need no entry.
