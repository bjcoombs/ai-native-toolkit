...
- **L5 - config snapshot drift:** take `config_drift.entries[0]` only when `config_drift.available` is true; fill from `file`, `key`, `tracked`, `live`. `available: false` means the comparison did not run (`reason` says why; `no_access` when the token cannot read the setting) - no evidence either way, so no row.
