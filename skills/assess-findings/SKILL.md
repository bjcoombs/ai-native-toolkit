...
- **Navigability, in words.** e.g. _"Of <N> docs, <P%> are reachable by following links from the README; the other <N> are orphans (nothing links to them) or sit in <K> disconnected islands."_ Pull `island_count`, `orphan_rate`, `reachability_pct`, and name 2-3 specific orphans/islands. These headline figures are the **curated wiki layer** (see the raw-source split below), so `doc_count` is `curated_doc_count`. **Frame it as curation, not access** - do not write "an agent can only discover <P%>": an agent can still `ls docs/` and open any file by path, so low link-reachability means the docs lack a navigable map (weak wayfinding / signal-vs-noise), not that content is hidden. State that adding an index/MOC is the fix, and present it as a wayfinding improvement, not a blocker - for a directory-organised tree, temper the priority accordingly.
