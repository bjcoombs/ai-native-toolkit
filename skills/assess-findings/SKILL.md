...
- **L0 - false instruction claim:** take `instruction_claims.failures[0]` when `instruction_claims.failed > 0`; fill from `file`, `line`, `path` and `kind` (`enforcement`: no workflow under `.github/workflows/` references the script; `pin`: the pinned file lacks `version` or does not exist). Name the count when more than one failed ("and <failed - 1> more").
