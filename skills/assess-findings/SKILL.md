...
- **Complexity profile:** per-function ccn p95 <N> (max <M>); file-aggregate ccn p95 <N> (max <M>); p95 est. tokens <N> (max <M>); p95 LOC <N> (max <M>; code <C>, data <D>, from `loc.max_code` / `loc.max_data`, data being scc's JSON, YAML and JSONL)
