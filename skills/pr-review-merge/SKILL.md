...
6. **Re-reviewing bots have reviewed the head SHA** - every bot the project's Marathon Configuration flags `Re-reviews on push: yes` has completed its pass on the current head SHA. For a bot configured with `Re-review check name`, only a check run of that name on the head SHA that completed with conclusion `success` counts (its submitted reviews are not consulted: the check run is what turns red when the reviewer exits with an error after posting). For a bot without one, its latest review must carry that head SHA as its `commit_id`. A review of an earlier commit does not count, and neither does a check run that is still in progress; any push restarts the wait. A check run that has completed with any conclusion other than `success` (failure, cancelled, skipped, neutral, timed_out) is a settled verdict: more waiting cannot change it, so stop waiting at once and take the warning path below instead of burning the max wait. The wait is bounded per bot by that bot's `Max wait for re-review`, measured from when the head commit was pushed (the earliest check-suite creation on the head SHA, not the commit's committer date, which predates the push and resets on rebase). If no check suite exists on the head SHA, fall back to the head commit's committer date as a floor - explicitly not the push time, it only guarantees the bound always expires. When the max wait expires before the bot completes, the criterion passes with a warning recorded in the merge record (the PR comment or report that accompanies the merge) naming the bot, the head SHA, and what was observed: for a check-name bot, the check run's conclusion (or its absence) on that SHA, and, so the record never asserts what the reviews endpoint refutes, whether path (a) shows a review of that SHA anyway; for a review bot, that no review of that SHA was found. The record states that the bot did not complete a green pass, not that it did not review. Bots without `Re-reviews on push: yes` are never waited on, so a project with no such flags sees no change.
...
gh api --paginate "repos/<owner>/<repo>/commits/$HEAD_SHA/check-runs?per_page=100" \
  | jq -r --arg n "<check-name>" '.check_runs[] | select(.name == $n and .status == "completed" and .conclusion == "success") | .completed_at'
```
Complete when, for a bot with `Re-review check name` set, path (b) prints at least one line (a green run; path (a) is not consulted for that bot), and otherwise when path (a) prints at least one line (a head-SHA review); non-empty output across all pages is the completion test. The filters use `--arg` and pipe to `jq` rather than `gh api --jq`, per the Shell Pitfalls below. Never match check runs by app slug: every GitHub Actions job, the reviewer included, reports as `github-actions`, so an unrelated job completing would pass the criterion. Waiting when neither path returns a match, no settled non-success check run exists for a check-name bot, and the max wait since `PUSHED_AT` has not expired; a settled non-success run ends the wait early with the same warning. A green reviewer check still warrants the spot-check the marathon skill describes (the reviewer's summary cites the head SHA) before merging on the strength of it.
