...
- **Recommended follow-up (out of this acceptance's scope):** to A/B an interactive target end-to-end, the runner harness needs **gate-scripting** - scripted user responses that drive a runner past sequential `AskUserQuestion` gates so post-gate behaviour is observed. That is an engine change to the runner contract (`skills/ab-equivalence/references/runner-prompt.md`) and is tracked as a separate follow-up, not made here.
